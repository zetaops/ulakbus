...
class KayitDondurmaForm(forms.JsonForm):
    ...
    baslangic_tarihi = fields.Date('Başlangıç Tarihi', required=True)

    class Donemler(ListNode):
        secim = fields.Boolean("Seçim", type="checkbox")
        donem = fields.String("Dönem", 'Donem')
        key = fields.String('Key', hidden=True)
        aciklama = fields.String('Açıklama', required=True)
...
class KayitDondurma(CrudView):
    ...
    def fakulte_yonetim_karar_no_gir(self):
        ...
        _form.karar = fields.String('Karar No', index=True, required=True)
    ...
    def donem_sec(self):
        ...
        if "kayit_dondurma_help_text" in self.current.task_data:
            ...
            _form.help_text = "Minimum 1 dönem, maximum 2 dönem kayıt dondurabilir."
    ...
    def ogrenci_kayit_dondur(self):
        ...
        if len(dondurulan_donemler) > 2 or len(dondurulan_donemler) < 1:
            text = "%s dönem seçtiniz. Minimum 1 dönem, maximum 2 dönem kayıt dondurabilir." % len(dondurulan_donemler)
            ...
            danisman_message = '%s numaralı, %s adlı öğrencinin %s programındaki kaydı ' \
            ...
            ogrenci_message = "%s dönemleri için kaydınız dondurulmuştur." % donemler

            ogrenci_program.ogrenci.user.send_notification(title="Kayıt Dondurma",
