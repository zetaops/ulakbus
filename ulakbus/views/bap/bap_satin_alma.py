...
class BAPSatinAlmaView(CrudView):
    def butce_kalemleri_sec_goster(self):
        self.current.task_data['obj_id'] = self.input.get('object_id') or self.current.task_data['obj_id']
        ...
        if msg:
            self.current.output['msgbox'] = {
                ...
                "msg": msg}
        butce_planlari = BAPButcePlani.objects.filter(ilgili_proje_id=self.current.task_data['obj_id'], satin_alma_durum=1)
