...
class TestCase(BaseTestCase):
    ...
    def test_setup(self):
        """
        Akademik takvim  iş akışı test edilmeden önce veritabanı boşaltılır,
        ...
        LoadData(path=os.path.join(os.path.expanduser('~'),
                                   'ulakbus/tests/fixtures/akademik_takvim.csv')).run()
        time.sleep(1)
    ...
    def test_academic_calendar(self):
        ...
        assert dict(AKADEMIK_TAKVIM_ETKINLIKLERI).get(str(etkinlik), '') == resp.json['object']['fields'][etkinlik - 1][
            'Etkinlik']
        assert takvim.baslangic.strftime('%d.%m.%Y') == resp.json['object']['fields'][etkinlik - 1][u'Başlangıç']
        assert takvim.bitis.strftime('%d.%m.%Y') == resp.json['object']['fields'][etkinlik - 1][u'Bitiş']
        ...
        assert response.json['object']['fields'][0]['Etkinlik'] == dict(AKADEMIK_TAKVIM_ETKINLIKLERI).get(
            str(etkinlik), '')
        assert response.json['object']['fields'][0][u'Başlangıç'] == takvim.baslangic.strftime('%d.%m.%Y')
